...
interface InputComponentTiptapProps {
  ...
  updateComponent: (index: number, newData: Partial<ComponentData>) => void;
}

const InputComponentTiptap: React.FC<InputComponentTiptapProps> = ({ id, content, coordinates, updateComponent }) => {
  ...
  return (
      ...
      >
        <div className="absolute bg-transparent inline-block max-w-full">
          ...
          {editor && (
            <div
              className={`toolbar flex space-x-2 mt-2 transition-opacity duration-300 ${position.y > window.innerHeight / 2 ? 'absolute bottom-full mb-2' : 'absolute top-full mt-2'}`}
              style={{ opacity: isEditorActive ? 1 : 0 }}
            >
              <button
                onClick={toggleBold}
                className={`w-8 h-8 rounded-full border border-black flex items-center justify-center ${editor.isActive('bold') ? 'bg-slate-400' : ''}`}
              >
                <strong>B</strong>
              </button>
              <button
                onClick={toggleItalic}
                className={`w-8 h-8 rounded-full border border-black flex items-center justify-center ${editor.isActive('italic') ? 'bg-slate-400' : ''}`}
              >
                <em>I</em>
              </button>
              <button
                onClick={toggleUnderline}
                className={`w-8 h-8 rounded-full border border-black flex items-center justify-center ${editor.isActive('underline') ? 'bg-slate-400' : ''}`}
              >
                <u>U</u>
              </button>
              <input
                ...
                className="w-8 h-8 rounded-full border border-black p-1"
                value={textColor}
              />
              <select
                onChange={handleFontSizeChange}
                className="px-1 border border-black flex items-center justify-center"
